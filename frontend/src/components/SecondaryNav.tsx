        ...
        <div className="flex space-x-6 overflow-x-auto scrollbar-hidepl-8 pr-8 items-center p-4 text-lg">
...

            {
                (editActive || tags.length == 0 ) && (
                    <Link to="/settings" className="text-slate-500 text-2xl mr-5 cursor-pointer hover:text-black">
                ...
                tags.map((tag: string) => {
                    return (
                        <div className={`relative ${editActive && "pt-3 pr-3"}`}>
                            ...
                            cursor-pointer text-slate-500 border-2 p-2 border-gray-300 hover:text-black border-2`} onClick={() => handleActiveTagChange(tag)}>
                                {tag}
                                {
                                    editActive && (
